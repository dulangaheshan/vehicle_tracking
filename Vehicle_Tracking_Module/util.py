...
def crop_for_feature_extraction(img, x1, y1, x2, y2, id, cam):
    print(img)
    print(id)
    print(x1, y1, x2, y2)
    ...
    cv2.imwrite(os.path.join('cropped' + "/" + cam, + str(id)+'.jpg'), img)
    # print(type(im))
    return True


def draw_bboxes(img, bbox, identities=None, camera=None, offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            executor.submit(crop_for_feature_extraction, img, x1, y1, x2, y2, id, camera)
